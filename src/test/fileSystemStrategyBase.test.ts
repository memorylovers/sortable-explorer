...
import { FileSystemStrategyBase } from "../explorer/strategies/fileSystemStrategyBase";
...
suite("FileSystemStrategyBase Tests", () => {
  ...
  test("同名のファイルが存在する場合、連番が付いたファイルが作成される", async () => {
    ...
    assert.strictEqual(
      datePattern.test(fileName2),
      true,
      "2つ目のファイル名に連番が付いていません"
    );
  });
  ...
  test("複数の同名ファイルが存在する場合、適切な連番が付いたファイルが作成される", async () => {
    ...
    assert.strictEqual(
      basePattern.test(fileName1),
      true,
      "1つ目のファイル名のフォーマットが正しくありません"
    );
    assert.strictEqual(
      /^\d{8}_MultipleNote_1\.md$/.test(fileName2),
      true,
      "2つ目のファイル名の連番が正しくありません"
    );
    assert.strictEqual(
      /^\d{8}_MultipleNote_2\.md$/.test(fileName3),
      true,
      "3つ目のファイル名の連番が正しくありません"
    );
  });
});
