...
// import { getWorkspaceFolders } from "../utils/workspaceUtils"; // 仮のユーティリティ関数
...
async function getWorkspaceFolders(): Promise<vscode.Uri[]> {
    // files.excludeの設定を取得
    const filesExcludeConfig = vscode.workspace.getConfiguration('files').get<{ [key: string]: boolean }>('exclude');
    const excludePattern = filesExcludeConfig ? `{${Object.entries(filesExcludeConfig)
    ...
    const allUris = await vscode.workspace.findFiles('**/*', excludePattern, 5000); // 上限を設定
    ...
    for (const uri of allUris) {
        ...
        for (const wf of workspaceFolders) {
            if (dirUri.fsPath.startsWith(wf.uri.fsPath)) {
                 // ルートに達するまで親フォルダを追加
                 while (dirUri.fsPath !== wf.uri.fsPath && dirUri.fsPath.length > wf.uri.fsPath.length) {
                    // 既にSetに含まれていれば、それ以上親を辿る必要はないかもしれない
                    if (folderUris.has(dirUri.toString())) {break;}
...
export async function moveFileCommand(item?: FileItem): Promise<void> {
  ...
  try {
      ...
      vscode.window.showErrorMessage(localize("moveFile.error.fetchFoldersFailed", error.message));
  ...
  // QuickPickアイテムを作成
  const quickPickItems: (vscode.QuickPickItem & { uri: vscode.Uri })[] = folders.map((folder) => {
    const relativePath = vscode.workspace.asRelativePath(folder, false);
    // workspaceFoldersが未定義の場合のフォールバックを追加
    const isRoot = workspaceFolders?.some(wf => wf.uri.toString() === folder.toString()) ?? false;
    const label = isRoot ? `$(folder-opened) ${path.basename(folder.fsPath)}` : `$(folder) ${path.basename(folder.fsPath)}`;
    // ルートフォルダの場合や相対パスが取得できない場合はフルパスを表示
    const description = relativePath && relativePath !== path.basename(folder.fsPath) ? relativePath : folder.fsPath; // 修正：ルートの場合フルパス
    return {
        label: label,
  ...
  // 移動先フォルダを選択
  const selectedItem = await vscode.window.showQuickPick<vscode.QuickPickItem & { uri: vscode.Uri }>(quickPickItems, {
  ...
  if (sourceUri.fsPath === destinationUri.fsPath || path.dirname(sourceUri.fsPath) === destinationFolderUri.fsPath) {
      vscode.window.showInformationMessage(localize("moveFile.info.sourceEqualsDestination"));
      return;
  }
  ...
  try {
    ...
    try {
      ...
      // ファイルが存在する場合、上書き確認
      const destinationDesc = vscode.workspace.asRelativePath(destinationFolderUri, false) || destinationFolderUri.fsPath;
      ...
      if (error instanceof vscode.FileSystemError && (error.code === 'FileNotFound' || error.code === 'ENOENT')) { // ENOENTも追加
          // OK、ファイルは存在しない
      } else {
    ...
    const destinationDesc = vscode.workspace.asRelativePath(destinationFolderUri, false) || destinationFolderUri.fsPath;
    vscode.window.showInformationMessage(localize("moveFile.success", sourceFileName, destinationDesc));

  } catch (error: any) {
    const destinationDesc = vscode.workspace.asRelativePath(destinationFolderUri, false) || destinationFolderUri.fsPath;
    vscode.window.showErrorMessage(
      localize("moveFile.error.moveFailed", sourceFileName, destinationDesc, error.message), // エラーメッセージにファイル名と移動先を追加
    );
    console.error(`Error moving file '${sourceFileName}' to '${destinationDesc}':`, error);
  }
}
